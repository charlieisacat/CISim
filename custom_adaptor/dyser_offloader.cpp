...
void moveInstructionAfter(Instruction *I, Instruction *TargetInst, std::unordered_map<llvm::Value*, std::vector<llvm::Value*>> &insnUsers)
{
    ...
    for (auto UI = I->use_begin(), UE = I->use_end(); UI != UE;)
    {
        ...
        if (User)
        {
            if (User->getParent()->getName() == I->getParent()->getName() && !isa<PHINode>(User))
            {
                ...
                {
                    // errs() << "       User=" << *User << "\n";
                    moveInstructionAfter(User, I, insnUsers);
                }
            }
        }
    }
    ...
    if(insnUsers.count(I))
    {
        auto users = insnUsers[I];
        for(auto user : users)
        {
            Instruction *insn = dyn_cast<Instruction>(user);
            if (insn)
            {
                if (insn->getParent()->getName() == I->getParent()->getName() && !isa<PHINode>(insn))
                {
                    if (!isLaterInBlock(insn, I))
                    {
                        // errs() << "       User=" << *User << "\n";
                        moveInstructionAfter(insn, I, insnUsers);
                    }
                }
            }
        }
...
void customize(int opcode,
               ...
               Module &M,
               LLVMContext &Context,
               unsigned int Threshold, std::unordered_map<llvm::Value*, std::vector<llvm::Value*>>& insnUsers)
{
    ...
    for (auto component : ret)
    {
        ...
        EncapsFuncType = FunctionType::get(ReturnType, InputTypes, false);
        ...
        for (unsigned i = 0; i < StructTypes.size(); ++i)
        {
            ...
            PackingFuncInputTypes.insert(PackingFuncInputTypes.begin(), Type::getInt64Ty(Context));
            ...
            Function *PackingFunc = Function::Create(PackingFuncType, Function::ExternalLinkage, "packing_function_" + std::to_string(i), &M);
            ...
            Values.push_back(MDString::get(Context, std::to_string(opcode)));
            MDNode *ComplexMetaNode = MDNode::get(Context, Values);
            PackingFunc->setMetadata("packing_func", ComplexMetaNode);
        ...
        for (unsigned i = 0; i < PackingFuncs.size(); ++i)
        {
            ...
            PackingFuncArgs.insert(PackingFuncArgs.begin(), ConstantInt::get((Type::getInt64Ty(Context)), StartIdx));
        ...
        for (Instruction *I : OutputInstructions.order)
        {
            FunctionType *ExtractFuncType = FunctionType::get(OutputTypes[Index], {Type::getInt64Ty(Context)}, false);
            Function *ExtractFunc = Function::Create(ExtractFuncType, Function::ExternalLinkage, "extract_func_" + BB->getName(), &M);
            Value *ExtractedValue = Builder.CreateCall(ExtractFunc, ConstantInt::get((Type::getInt64Ty(Context)), Index++));

            for(auto call : PackedStructs){
                if(!insnUsers.count(call)){
                    insnUsers[call] = std::vector<llvm::Value*>();
                }

                insnUsers[call].push_back(ExtractedValue);
            }

            // MDNode *MetadataNode = MDNode::get(Context, MDString::get(Context, std::to_string(opcode)));
            // ExtractFunc->setMetadata("extract_func", MetadataNode);
            std::vector<Metadata *> Values;
            Values.push_back(MDString::get(Context, std::to_string(opcode)));
            MDNode *ComplexMetaNode = MDNode::get(Context, Values);
            ExtractFunc->setMetadata("extract_func", ComplexMetaNode);
                    ...
                    if (std::find(InstructionsToMove.begin(), InstructionsToMove.end(), User) == InstructionsToMove.end())
                    {
                        ...
                        if (!isLaterInBlock(User, tmp))
                        {
                            ...
                            if (!isa<PHINode>(User) && User->getParent()->getName() == tmp->getParent()->getName())
                                moveInstructionAfter(User, tmp, insnUsers);
                        }
                    }
                }
                else
...
int main(int argc, char **argv)
{
    ...
    for (auto &F : *M)
    {
        ...
        // keep dependency of extract_func and packing_func
    std::unordered_map<llvm::Value*, std::vector<llvm::Value*>> insnUsers;

    int n = 0;
    std::vector<Candidate> candidates;
    std::set<std::string> banBB;
    ...
    for (const auto &entry : fs::directory_iterator(patternDir))
    {
        if (entry.is_regular_file())
        {
            ...
            for (int i = 0; i < candidate.size(); i++)
            {
                ...
                if (compnt->vertices.size() > max_size)
                {
                    max_size = compnt->vertices.size();
                    target = i;
                }
            }
            if (!candidate.size())
            ...
            // FIX THIS: only support one candidate component per bb now
            // if (banBB.count(candidate[target]->bbName))
            // {
            //     continue;
            // }

            banBB.insert(candidate[target]->bbName);

            Candidate temp;
            temp.push_back(candidate[target]);
            // std::cout<<"opcode="<<opcode<<"\n";
            // customize(opcode, temp, instructions, *M, Context, Threshold, insnUsers);
            customize(opcode, candidate, instructions, *M, Context, Threshold, insnUsers);
