...
std::vector<int> customize(int opcode,
                           Candidate candidate,
                           ...
                           unsigned int Threshold)
{
    ...
    for (auto component : ret)
    {
        ...
        if (!noRequirePacking)
        {
            ...
            for (unsigned i = 0; i < StructTypes.size(); ++i)
            {
                ...
                MDNode *MetadataNode = MDNode::get(Context, MDString::get(Context, "PackingFunction"));
        ...
        packing_insn_n += PackingFuncs.size();
    ...
    return {packing_insn_n, encap_insn_n};
}
...
int main(int argc, char **argv)
{
    ...
    int opcode = 1024;
    int n = 0;
    std::vector<int> encap_insn_n_vec;
    // errs() << patternDir << "\n";
    ...
    std::sort(candidates.begin(), candidates.end(), [](const Candidate &a, const Candidate &b)
              { return a[0]->vertices.size() > b[0]->vertices.size(); });

    for (auto candidate : candidates)
    {
        int opcode = candidate[0]->opcode;
        auto data = customize(opcode, candidate, instructions, *M, Context, Threshold);
        if (data.size() != 0)
        {
            encap_insn_n_vec.push_back(data[1]);
        }
    }
    ...
    for (int i = 0; i < n + 2; i++)
    {
        ...
        if (i == 0)
            out << YAML::Key << "encap_n" << YAML::Value << i;
        else
            out << YAML::Key << "encap_n" << YAML::Value << encap_insn_n_vec[i - 1];
        out << YAML::EndMap;
    }
    ...
    out << YAML::Key << "encap_n" << YAML::Value << all_packing_insn_n;
    out << YAML::EndMap;
    ...
    for (int i = 1; i < n + 1; i++)
    {
        ...
        out << YAML::BeginMap;
        out << YAML::Key << "ports" << YAML::Value << i;
        out << YAML::Key << "fu" << YAML::Value << i;
        out << YAML::Key << "opcode_num" << YAML::Value << 1023 + i;
    ...
    if (fout.is_open())
    {
        fout << out.c_str();
        fout.close();
        std::cout << "YAML data has been written to output.yaml" << std::endl;
    }
    else
    {
        std::cerr << "Failed to open output.yaml for writing." << std::endl;
    }
