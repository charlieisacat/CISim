...
pair<double, vector<int>> knapsack_01_with_items(int W, vector<int> &weights, vector<double> &values)
{
    int n = weights.size();
    ...
    for (int i = 1; i <= n; ++i)
    {
        for (int j = 0; j <= W; ++j)
        {
            ...
            {
                double take = dp[i - 1][j - weights[i - 1]] + values[i - 1];
                if (take > dp[i - 1][j])
                {
                    dp[i][j] = take;
                    selected[i][j] = true;
    ...
    for (int i = n; i >= 1; --i)
    {
        ...
        {
            chosen.push_back(i - 1); // 记录原始索引（0-based）
...
int main(int argc, char **argv)
{
    ...
    double ratio = std::stod(argv[9]);
    int portMax = std::stoi(argv[10]);

    std::cout << "portMax=" << portMax << "\n";
    ...
    for (auto &F : *M)
    {
        for (auto &BB : F)
        {
            ...
            for (auto &I : BB)
            {
                if (count(bbList.begin(), bbList.end(), bbName))
                {
                    bb_insn++;
                }
    ...
    for (const auto &entry : fs::directory_iterator(patternDir))
    {
        if (entry.is_regular_file())
        {
            ...
            for (int i = 0; i < candidate.size(); i++)
            {
                ...
                // std::cout << "nbb=1" << " ninsn=" << compnt->vertices.size() << "\n";
            }
            if (!candidate.size())
                continue;

            candidates.push_back(candidate);
            areas.push_back(op_area);
            total_area += op_area;
    ...
    cout << "========= " << bb_insn << " " << cstn_insn << "\n";

    assert(candidates.size() == areas.size());
    cout<<"size === "<<candidates.size()<<"\n";
    vector<double> values;
    for (auto candidate : candidates)
    {
        values.push_back(candidate[0]->score);
        // cout<<"score= "<<candidate[0]->score<<"\n";
    }

    int W = (int)round(total_area * ratio);
    auto [max_val, items] = knapsack_01_with_items(W, areas, values);
    // cout << "W=" << W << " total_area=" << total_area<<" item.size()="<<items.size()<<" m = "<<max_val << "\n";

    std::vector<Candidate> new_candidates = candidates;
#if 0
    std::vector<Candidate> new_candidates;
    std::vector<int> new_areas;
    for (auto item : items)
    {
        // cout << "max_val=" << max_val << " item=" << item << "\n";
        new_candidates.push_back(candidates[item]);
        new_areas.push_back(areas[item]);
    }
#endif
    ...
    {
        // cout<<"opcode="<<opcode<<" area="<<new_areas[i]<<"\n";
    ...
    for (int i = 0; i < n; i++)
    {
        std::string customInsn = "custom_insn" + std::to_string(i);
        // std::cout << "customInsn = " << customInsn << " ??? " << 2 + i % portMax << "\n";
        out << YAML::Key << customInsn;
        out << YAML::BeginMap;
        out << YAML::Key << "ports" << YAML::Value << 2 + i % portMax;
        out << YAML::Key << "fu" << YAML::Value << 2 + i;
        out << YAML::Key << "opcode_num" << YAML::Value << 1025 + i;
